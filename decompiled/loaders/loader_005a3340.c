/* spd-match: far pct=65.00 flags=/O2 /c /nologo /TC method=m39 source=m39/4.1.G.4-float/va_005A3340 */
...
typedef unsigned int uint; typedef int int32; uint __fastcall FUN_005a3340(void *ecx) { int count = *(int32 *)((char *)ecx + 0x5868); int i = 0; char *esi = (char *)ecx + 4; if (count <= 0) return 1; do { char *target = ((i < 0) || (i >= count) || (i > 7)) ? 0 : esi; if (*(int32 *)(target + 0x14) < 3) return 0; i++; esi += 0xb0c; } while (i < count); return 1; }
