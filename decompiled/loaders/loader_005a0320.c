/* spd-match: far pct=7.32 flags=/O1 /c /nologo /TC method=m39 source=m39/4.1.G.4-float/va_005A0320 */
...
typedef unsigned int uint; uint __fastcall FUN_005a0320(int *base) { int count = *(int*)((char*)base + 0x74); int i = 0; if (count > 0) { int *curr = (int*)((char*)base + 0x38); do { if (curr[0] == 0 && curr[2] == 4) return 1; i++; curr += 3; } while (i < count); } return 0; }
