/* spd-match: far pct=4.26 flags=/O1 /c /nologo /TC method=m39 source=m39/4.1.G.1-filter100/va_0059E5F0 */
...
int unaff_EBX;
typedef unsigned int uint; struct Context { int count; char *data; }; uint __fastcall FUN_0059e5f0(struct Context *ctx, int ebx) { uint eax = 0; int edx = ctx->count; char *esi = ctx->data; if (edx <= 0) return 0; do { int *p = (int *)(esi + 0x24); if (p[-1] == 2 && p[0] == ebx) return (unsigned char)*(esi + (eax * 0x34) + 0x30); eax++; esi += 0x34; } while ((int)eax < edx); return 0; }
