/* spd-match: far pct=9.30 flags=/O2 /c /nologo /TC method=m39 build_strip source=m39/3.9.26.5/va_004E0BF0 */
...
uint __fastcall FUN_004e0bf0(uint val, int param_1)

{
  ...
  if ((val & 1) != 0) {
    return (uint)((&DAT_00748f78)[param_1] == 0);
  }
  if ((val & 2) != 0) {
    return (uint)((&DAT_00748f78)[param_1] == 1);
  }
  if ((val & 4) != 0) {
    return (uint)((&DAT_00748f78)[param_1] == 2);
  }
  iVar1 = (-(uint)((val & 8) != 0) & 4) - 1;
  return CONCAT31((int3)((uint)iVar1 >> 8),iVar1 == (&DAT_00748f78)[param_1]);
}
