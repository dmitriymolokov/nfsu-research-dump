/* spd-match: far pct=9.30 flags=/O1 /c /nologo /TC method=m4_12_mid_void source=m4_12/4.1.2/va_0054CFE0 */
/* Pass A — minimal Ghidra → MSVC6 typedefs (not from speed.exe) */
/* M4.1.1: unaff_EDI → __fastcall typed arg (promote; sync only on uplift). */
...
void __fastcall FUN_0054cfe0(int edi)

{
  ...
  FUN_00549500();
  if ((*(int *)p08_s_tsmiInvalid_master_account_name_006f2f38) != -1) {
    pcVar2 = p08_s_tsmiInvalid_master_account_name_006f2f38;
    iVar1 = (*(int *)p08_s_tsmiInvalid_master_account_name_006f2f38);
    do {
      if (iVar1 == edi) {
        return;
      }
      iVar1 = *(int *)(pcVar2 + 0x2c);
      pcVar2 = pcVar2 + 0x2c;
    } while (iVar1 != -1);
  }
  return;
