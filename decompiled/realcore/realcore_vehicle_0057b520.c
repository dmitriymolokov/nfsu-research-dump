/* spd-match: far pct=29.55 flags=/O1 /c /nologo /TC method=m39 source=m39/m39/va_0057B520 */
...
struct Entry { int data[0x38/4]; }; int __fastcall FUN_0057b520(int edi, int obj) { int count = *(int*)(obj + 4); struct Entry* p = (struct Entry*)(obj + 0x3c); int i = 0; if (count <= 0) return 0; while (i < count) { if (p->data[0] == edi) { if (i == -1) return 0; return i * 0x38 + 8 + obj; } i++; p++; } return 0; }
