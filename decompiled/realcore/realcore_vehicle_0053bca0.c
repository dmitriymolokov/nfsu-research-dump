/* spd-match: close pct=86.21 flags=/O2 /c /nologo /TP method=m39 source=m39/m39/va_0053BCA0 */
...
/* M4.1.3: FUN_0064b7d0 → __stdcall (callee_stdcall). */
...
int __stdcall FUN_0064b7d0(int *p);
int __fastcall FUN_0053bca0(int *ecx_val)
{
  int eax_val = 0;
  int *p = ecx_val;
  if (*p != 0) {
    FUN_0064b7d0(&eax_val);
  }
  return eax_val;
}
