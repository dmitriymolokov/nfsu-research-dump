/* spd-match: far pct=17.14 flags=/O1 /c /nologo /TC method=m39 source=m39/m39/va_00572280 */
...
int unaff_ESI;
typedef unsigned int uint; typedef int int32; struct Container { int reserved[42]; int count; int index; int data[1024]; }; int32 __fastcall FUN_00572280(struct Container *ecx, int esi) { int eax = 0; int edx = ecx->index; if (edx != 0 && ecx->data[edx] < esi) eax = edx; if (eax < ecx->count) { int *ptr = &ecx->data[eax]; while (eax < ecx->count) { if (*ptr == esi) { ecx->index = eax; return 1; } eax++; ptr++; } } return 0; }
