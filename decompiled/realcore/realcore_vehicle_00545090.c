/* spd-match: far pct=12.82 flags=/O2 /c /nologo /TC method=m39 source=m39/m39/va_00545090 */
...
typedef struct Node { struct Node *next; struct Node *prev; } Node; void __cdecl _free(void *); void __fastcall FUN_00545090(Node **head) { Node *curr = *head; while (curr != (Node *)head) { Node *next = curr->next; Node *prev = curr->prev; prev->next = next; next->prev = prev; _free((char*)curr - 4); curr = *head; } }
