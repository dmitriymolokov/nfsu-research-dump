#pragma once
/* Ghidra → C soft-compat. Written for this research dump — not from Speed.exe.
 * Do not #include <windows.h> on match TUs. Per-VA FUN_/DAT_ stubs stay in the .c file.
 * See docs/EAX_IN_EAX.md.
 */
/* Clang / non-MSVC: calling-convention keywords + __int64. */
#if !defined(_MSC_VER)
#ifndef __int64
...
/* /TC: () stubs; /TP: code(...) via __cplusplus. bool is keyword in C++. */
...
typedef int             int7;
#ifdef __cplusplus
typedef int             code(...); /* /TP thiscall rewrite */
#else
...
/* MXCSR / ExceptionList: Wine/cl is single-TU (tentative def OK). Host multi-TU
 * link needs one definition in host/stubs/globals_host.c — compile host with
 * -DSPD_HOST_RUNTIME. */
#if defined(SPD_HOST_RUNTIME)
extern unsigned int MXCSR;
